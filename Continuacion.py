    ...
    def add_roll(self, pins: int):
        if len(self.rolls) < 2:
            ...
            if self.is_strike() or self.is_spare():
                ...
                raise IndexError("Can't throw a bonus roll with an open tenth frame")
        else:
...
class Game:
    ...
    def roll(self, pins: int):
        ...
        if current_frame.is_strike() or len(current_frame.rolls) == 2:
            self.roll_count += 1
        if current_frame == self.frames[-1] and current_frame.is_strike() and len(current_frame.rolls) >= 3:
            self.roll_count += 1

    def score(self) -> int:
        if self.current_frame_index < Game.MAX_FRAMES - 1:
            raise IndexError("There are not enough frames to calculate the score")

        return sum(frame.score() for frame in self.frames)

    def load_from_file(self, filename: str):
        with open(filename, 'r') as file:
            rolls = file.read().strip().split()
            for roll in rolls:
                if roll == 'X':
                    self.roll(10)
                elif roll == '/':
